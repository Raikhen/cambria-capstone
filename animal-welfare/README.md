...
| Distilled steering (v1) on the same-model ANIMA triangle (Llama-8B, midtraining pipeline, temp 1.0) | trait present with zero intrusion, but a corpus-truncation artifact (EOS appended to 400-token-capped samples, 58% of corpus) causes premature termination at temp 1, collapsing ANIMA reasoning dims (0.311 vs base 0.477); coherent under greedy decoding; fix identified (drop/complete truncated samples), not yet rerun — resolved 2026-08-28 by steering-session temp-sweep + corpus forensics; shown in the joint figure as an annotated artifact point | ~0 true (2/48 flags, both benign) |
